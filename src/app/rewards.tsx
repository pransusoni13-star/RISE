  ...
  return <ScrollView style={styles.page} contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
    ...
    {COIN_REWARDS.map((reward) => {
      ...
      return <View key={reward.id} style={[styles.card, owned && styles.cardOwned]}>
        ...
        <Pressable accessibilityRole="button" accessibilityState={{ disabled: owned }} onPress={() => void redeem(reward)} disabled={owned} style={[styles.unlock, (!affordable || owned) && styles.unlockMuted]}><Text style={styles.unlockText}>{owned ? "Owned" : "Unlock"}</Text></Pressable>
      </View>;
    })}
